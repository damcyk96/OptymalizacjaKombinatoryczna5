...
module.exports = {
  einConstructor: function () {
    ...
    function pre(a, b) {
      ...
      for (let piece of puzzleA.pieces) puzzleB.pre.add(piece.value);
      for (let piece of puzzleB.pieces) puzzleA.after.add(piece.value);
    }
    ...
    function neighbours(a, b, preFlag = false) {
      ...
      for (let piece of puzzleA.pieces) puzzleB.neighbours.add(piece.value);
      for (let piece of puzzleB.pieces) puzzleA.neighbours.add(piece.value);
    ...
    function joinPuzzles(a, b) {
      ...
      for (let pval of b.pre) {
        if (a.after.has(pval))
          throw new Error("Pre/after conflict when joining two puzzles.");
        a.pre.add(pval);
      }
